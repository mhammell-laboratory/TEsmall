...
def get_stat(prefix, maxaln):
    ...
    df = pd.concat([df, pd.read_csv("{0}.3trf.TE.mapper.anno".format(prefix)], sep="\t", usecols=["rid"]), ignore_index=True)
    df = pd.concat([df, pd.read_csv("{0}.3trf.struc.mapper.anno".format(prefix)], sep="\t", usecols=["rid"]), ignore_index=True)
    anno_reads = len(df.rid.unique())
