...
def EMUpdate(meansIn, te_len, te_idx, uniq_counts, multi_reads, estimatedReadLength):
    # reassign multi-reads proportionally to the relative abundance of each TE
    te_counts = {}
    meansOut = [0] * len(meansIn)

    multi_counts = computeAbundances(meansIn, multi_reads, te_idx)
...
def expectedLogLikelihood_(means, counts, te_len):
    sampProbs = [0.0] * len(means)

    for tid in range(len(counts)):
        relativeAbundance = means[tid]
   #     sampProbs[tid] = te_len[tid] * relativeAbundance
...
def EMestimate(te_len, multi_reads, te_idx, uniq_counts, multi_counts, numItr, estimatedReadLength):  # input from annotation.py
   ...
   # estimatedReadLength = averageReadLength(multi_reads)
    ...
    for tid in range(len(uniq_counts)):
        tlen = te_len[tid]  # check for other instances of te_len
        if tlen < 0:
            sys.stderr.write("Error in optimization: the TE does not exist!\n")
            raise
        effectiveLength = tlen - estimatedReadLength + 1
    ...
    while cur_iter < numItr:
        ...
        try:
            means1 = EMUpdate(means0, te_len, te_idx, uniq_counts, multi_reads, estimatedReadLength)
        except:
        ...
        try:
            means2 = EMUpdate(means1, te_len, te_idx, uniq_counts, multi_reads, estimatedReadLength)
        except:
            sys.stderr.write("Error in EMupdate\n")
            raise
...
#        delta = absdiff_(means1, means2)
 #       sys.stderr.write("delta = " + delta + "\n")
        ...
        if abs(alphaS - 1.0) > 0.01:
            ...
            try:
                meansPrime = EMUpdate(meansPrime, te_len, te_idx, uniq_counts, multi_reads, estimatedReadLength)
            except:
                sys.stderr.write("Error in EMupdate\n")
                raise
    ...
    new_multi_counts = computeAbundances(means0, multi_reads, te_idx)
    return new_multi_counts


def computeAbundances(meansIn, multi_reads, te_idx):

    size = len(meansIn)
    multi_counts = [0] * size
    # invert dict to make te_names keys
    inv_te_idx = {v: k for k, v in te_idx.items()}
    ...
    for kid in range(len(multi_reads)):
        ...
        for tid in TE_transcripts:
            totalMass += meansIn[inv_te_idx[tid]]
        ...
        for tid in TE_transcripts:
            multi_counts[inv_te_idx[tid]] += meansIn[inv_te_idx[tid]] * norm
